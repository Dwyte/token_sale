...
contract('DappToken', function(accounts){
	...
	it('initializes the contract with the correct values', function(){
		return DappToken.deployed().then(function(instance){
			...
			assert.equal(name, 'Marty Token', 'has the correct name');
			return tokenInstance.symbol()
		}).then(function(symbol){
			assert.equal(symbol, 'Mrty', 'has the correct symbol');
			return tokenInstance.standard();
		}).then(function(standard){
			assert.equal(standard, 'Marty Token v1.0', 'has the correct standard');
		})
	})
	...
	it('allocates the initial supply upon deployment', function(){
		return DappToken.deployed().then(function(instance){
			...
			assert.equal(adminBalance.toNumber(), 1000000, 'it allocates to the initial supply');
		});
	});
    ...
    return DappToken.deployed().then(function(instance) {
    	...
    	return tokenInstance.transferFrom.call(fromAccount, toAccount, 10, { from: spendingAccount });
    		...
    		assert.equal(balance.toNumber(), 90, 'deducts from the sending account');
    		return tokenInstance.balanceOf(toAccount);
    	}).then(function(balance){
    		assert.equal(balance.toNumber(), 10, 'adds the amount to the recieving account');
    		return tokenInstance.allowance(fromAccount, spendingAccount);
    	}).then(function(allowance){
    		assert.equal(allowance.toNumber(), 0, 'updates allowance');
    	});
  	});
});